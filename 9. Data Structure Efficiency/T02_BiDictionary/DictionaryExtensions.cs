...
public static class DictionaryExtensions
{
    ...
    /// If the key is missng, an empty sequence is returned.
    /// </summary>
    public static IEnumerable<TValue> GetValuesForKeys<TKey1, TKey2, TValue>
        (this IDictionary<Tuple<TKey1, TKey2>, List<TValue>> dict, TKey1 key1, TKey2 key2)
    {
        List<TValue> collection;
        if (dict.TryGetValue(new Tuple<TKey1, TKey2>(key1, key2), out collection))
        {
            return collection;
        }
        return Enumerable.Empty<TValue>();
